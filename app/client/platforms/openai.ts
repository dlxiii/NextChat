...
export class ChatGPTApi implements LLMApi {
   ...
   * 3) If yes and provider `Authorization` is still missing, use signed-in session token as
  ...
  private getChatHeaders(chatPath: string) {
    ...
    const hasProviderAuthorization = Boolean(headers["Authorization"]?.trim());
    if (hasProviderAuthorization) {
      return headers;
    }
